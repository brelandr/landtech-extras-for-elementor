...
class Extension_Display_Conditions extends Extension_Base {
	...
	protected function add_common_sections_actions() {
		...
		// Activate sections for sections (legacy layouts)
		add_action( 'elementor/element/section/section_custom_css/after_section_end', function( $element, $args ) {
			$this->add_common_sections( $element, $args );
		}, 10, 2 );

		// Flexbox / Grid containers
		add_action( 'elementor/element/container/section_custom_css/after_section_end', function( $element, $args ) {
			$this->add_common_sections( $element, $args );
		}, 10, 2 );
		...
		add_action( 'elementor/element/container/section_custom_css_pro/after_section_end', function( $element, $args ) {
			$this->add_common_sections( $element, $args );
		}, 10, 2 );
