...
class Licensing {
	...
	public function add_actions() {
		if ( is_admin() ) {
			...
			add_action( 'admin_post_elementor_extras_save_lmfwc_rest', 		array( $this, 'save_lmfwc_credentials' ) );
	...
	public static function get_status_errors() {
		...
		return [
			self::STATUS_EXPIRED => [
				...
				'action'		=> __( '<a href="%s" target="_blank">Renew your license</a> to keep getting updates and access to premium support.', 'elementor-extras' ),
				'link'			=> esc_url( apply_filters( 'elementor_extras_license_account_url', 'https://landtechwebdesigns.com', self::get_license_key() ) ),
				'label'			=> 'expired',
				'dismissable' 	=> [
					'key' 		=> 'license-expired',
					'duration' 	=> 7,
			...
			self::STATUS_LIMIT 	=> [
				...
				'link'			=> esc_url( apply_filters( 'elementor_extras_license_upgrade_url', 'https://landtechwebdesigns.com', self::get_license_key() ) ),
				'label'			=> 'limit reached',
			],
		];
	}
	...
	/**
	 * License display state from LMFWC / LandTech validation (option `elementor_extras_license_status`).
	 *
	 * @param bool $force_request When true, re-validates with the store before reading the option.
	 * @return array<string,mixed>
	 * @since 2.1.0
	 */
	public static function get_license_data( $force_request = false ) {
		if ( $force_request && class_exists( 'ElementorExtras_License_Cron' ) ) {
			ElementorExtras_License_Cron::validate_stored_license( 'manual' );
		}

		$st = get_option( 'elementor_extras_license_status', '' );

		if ( 'valid' === $st ) {
			return array(
				'license'           => self::STATUS_VALID,
				'expires'           => 'lifetime',
				'subscriptions'     => '',
				'site_count'        => '0',
				'activations_left'  => '',
	...
	/**
	 * Former EDD transient cache (no longer used).
	 *
	 * @param array<mixed>|mixed $license_data Unused.
	 * @param int|null           $expiration  Unused.
	 * @return void
	 */
		...
		<div class="wrap">
			...
			<form method="post" action="<?php echo esc_url( admin_url( 'admin-post.php' ) ); ?>">
				...
				<div class="metabox-holder ee-metabox-holder ee-settings">
						...
						<tbody>
							...
							?><tr valign="top" class="ee-form-table__row">
								...
								<td class="ee-form-table__cell ee-form-table__cell--strong">
									...
									<?php elseif ( isset( $license_data['license'] ) && 'inactive' === $license_data['license'] ) : ?>
										<span style="color: #996800;"><?php _e( 'Pending verification', 'elementor-extras' ); ?></span>
									<?php else : ?>
					...
					<?php
					$key_const = function_exists( 'elementor_extras_lmfwc_consumer_key_from_constant' ) && elementor_extras_lmfwc_consumer_key_from_constant();
					$sec_const = function_exists( 'elementor_extras_lmfwc_consumer_secret_from_constant' ) && elementor_extras_lmfwc_consumer_secret_from_constant();
					?>

					<h2 class="title" style="margin-top:24px"><?php esc_html_e( 'License Manager REST API credentials', 'elementor-extras' ); ?></h2>
					<p class="description"><?php esc_html_e( 'WooCommerce REST keys that can access License Manager (LMFWC). Values are encrypted in the database. Optional: define ELEMENTOR_EXTRAS_CONSUMER_KEY and ELEMENTOR_EXTRAS_CONSUMER_SECRET in wp-config.php instead.', 'elementor-extras' ); ?></p>

					<?php if ( $key_const || $sec_const ) : ?>
						<p class="notice notice-info inline"><?php esc_html_e( 'One or both credentials are overridden from wp-config.php; fields below cannot replace them.', 'elementor-extras' ); ?></p>
					<?php endif; ?>

					<form method="post" action="<?php echo esc_url( admin_url( 'admin-post.php' ) ); ?>" style="margin-top:12px;">
						<?php wp_nonce_field( 'elementor_extras_lmfwc_rest', 'elementor_extras_lmfwc_nonce' ); ?>
						<input type="hidden" name="action" value="elementor_extras_save_lmfwc_rest" />
						<input type="hidden" name="_wp_http_referer" value="<?php echo esc_attr( isset( $_SERVER['REQUEST_URI'] ) ? wp_unslash( $_SERVER['REQUEST_URI'] ) : '' ); ?>" />
						<table class="form-table ee-form-table ee-form-table--compact">
							<tbody>
								<tr>
									<th scope="row"><label for="ee_lmfwc_key"><?php esc_html_e( 'Consumer key', 'elementor-extras' ); ?></label></th>
									<td><input name="ee_lmfwc_key" id="ee_lmfwc_key" type="text" class="regular-text code" value="" autocomplete="off" <?php disabled( $key_const ); ?> /></td>
								</tr>
								<tr>
									<th scope="row"><label for="ee_lmfwc_secret"><?php esc_html_e( 'Consumer secret', 'elementor-extras' ); ?></label></th>
									<td><input name="ee_lmfwc_secret" id="ee_lmfwc_secret" type="password" class="regular-text code" value="" autocomplete="off" <?php disabled( $sec_const ); ?> /></td>
								</tr>
							</tbody>
						</table>
						<?php submit_button( __( 'Save REST credentials', 'elementor-extras' ), 'secondary', 'submit', false ); ?>
					</form>
		</div><?php
	}
	...
	public function activate_license() {
		...
		if ( empty( $_POST['elementor_extras_license_key'] ) ) {
			wp_die( esc_html__( 'Please enter your license key.', 'elementor-extras' ), esc_html__( 'Extras for Elementor', 'elementor-extras' ), array( 'back_link' => true ) );
		}

		$license_key = sanitize_text_field( wp_unslash( $_POST['elementor_extras_license_key'] ) );

		$block = '';
		if ( function_exists( 'elementor_extras_landtech_license_precheck_activation_blocked_message' ) ) {
			$block = elementor_extras_landtech_license_precheck_activation_blocked_message(
				$license_key,
				elementor_extras_get_license_server_url(),
				elementor_extras_get_license_product_id(),
				elementor_extras_get_license_shared_secret()
			);
		}
		if ( '' !== $block ) {
			wp_die( wp_kses_post( $block ), esc_html__( 'Extras for Elementor', 'elementor-extras' ), array( 'back_link' => true ) );
		}

		if ( ! class_exists( 'ElementorExtras_License_Cron' ) ) {
			wp_die( esc_html__( 'Licensing components failed to load.', 'elementor-extras' ), esc_html__( 'Extras for Elementor', 'elementor-extras' ), array( 'back_link' => true ) );
		}

		$activate_result = ElementorExtras_License_Cron::activate_via_lmfwc( $license_key );
		if ( null === $activate_result ) {
			wp_die(
				esc_html__(
					'Could not reach the license server, or REST credentials are missing. Save the LMFWC consumer key and secret below (or define ELEMENTOR_EXTRAS_CONSUMER_KEY and ELEMENTOR_EXTRAS_CONSUMER_SECRET in wp-config.php) and confirm Settings → General has your license server URL, product ID, and shared secret.',
					'elementor-extras'
				),
				esc_html__( 'Extras for Elementor', 'elementor-extras' ),
				array( 'back_link' => true )
			);
		}
		if ( false === $activate_result ) {
			wp_die(
				esc_html__( 'This license key was not accepted by the store (License Manager).', 'elementor-extras' ),
				esc_html__( 'Extras for Elementor', 'elementor-extras' ),
				array( 'back_link' => true )
			);
		}
		...
		wp_safe_redirect( esc_url_raw( wp_unslash( $_POST['_wp_http_referer'] ?? admin_url() ) ) );
		exit;
	}
	...
	public function deactivate_license() {
		...
		delete_transient( 'elementor_extras_license_data' );
		self::delete_landtech_license_snapshot_options();
	...
	/**
	 * Save encrypted LMFWC REST credentials (consumer key / secret).
	 *
	 * @return void
	 */
	public function save_lmfwc_credentials() {
		if ( ! current_user_can( 'manage_options' ) ) {
			return;
		}
		check_admin_referer( 'elementor_extras_lmfwc_rest', 'elementor_extras_lmfwc_nonce' );

		$referer = isset( $_POST['_wp_http_referer'] ) ? esc_url_raw( wp_unslash( $_POST['_wp_http_referer'] ) ) : admin_url( 'admin.php?page=' . $this->get_settings_page_slug() );

		if ( function_exists( 'elementor_extras_lmfwc_consumer_key_from_constant' ) && elementor_extras_lmfwc_consumer_key_from_constant()
			&& function_exists( 'elementor_extras_lmfwc_consumer_secret_from_constant' ) && elementor_extras_lmfwc_consumer_secret_from_constant() ) {
			wp_safe_redirect( $referer );
			exit;
		}

		$key = isset( $_POST['ee_lmfwc_key'] ) ? sanitize_text_field( wp_unslash( $_POST['ee_lmfwc_key'] ) ) : '';
		$sec = isset( $_POST['ee_lmfwc_secret'] ) ? sanitize_text_field( wp_unslash( $_POST['ee_lmfwc_secret'] ) ) : '';

		if ( '' !== $key ) {
			update_option( 'elementor_extras_consumer_key_encrypted', elementor_extras_encrypt( $key ), false );
		}
		if ( '' !== $sec ) {
			update_option( 'elementor_extras_consumer_secret_encrypted', elementor_extras_encrypt( $sec ), false );
		}
	...
	/**
	 * Clear entitlement snapshot options when the license is removed locally.
	 *
	 * @return void
	 */
	private static function delete_landtech_license_snapshot_options() {
		$keys = array(
			'elementor_extras_landtech_subscription_status',
			'elementor_extras_landtech_eligible_for_updates',
			'elementor_extras_landtech_reason_code',
			'elementor_extras_landtech_purchase_type',
			'elementor_extras_landtech_is_revoked',
		);
		foreach ( $keys as $opt ) {
			delete_option( $opt );
		}
