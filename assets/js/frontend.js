( function( $, window ) {
	...
	var ee = { 
		...
		/**
		 * Direct child .elementor-widget-container is omitted when Elementor Optimized Markup is active.
		 */
		getWidgetContentRoot : function ( $widgetScope ) {
			var $inner = $widgetScope.children( '.elementor-widget-container' );
			return $inner.length ? $inner : $widgetScope;
		},
		...
		fixSwipers : function( $container ) {
			var $swipers = $container.find( '.swiper-container-initialized, .swiper-initialized' );
		...
		Switcher : function( $scope, $ ) {
			...
			ee.Switcher.init = function() {
				...
				switch ( ee.Switcher.elementSettings.background_switcher_element ) {
					case 'widget':
						switcherArgs.background = ee.getWidgetContentRoot( $scope );
						break;
					case 'section':
		...
		PostsCarousel : function( $scope, $ ) {
			...
			ee.onElementRemove( $scope, function() {
				$scope.find('.ee-swiper__container').each( function() {
					if ( $(this).data('swiper') ) {
		...
		ParallaxElement : function( $scope, $ ) {
			...
			ee.ParallaxElement.init = function() {
				...
				if ( 'widget'=== ee.getElementType( $scope ) ) $element = ee.getWidgetContentRoot( $scope );
