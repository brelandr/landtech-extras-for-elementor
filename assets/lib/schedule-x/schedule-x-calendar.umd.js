(function (global, factory) {
  ...
  function ViewSelection() {
      ...
      return (jsxRuntime.jsxs("div", { className: `sx__view-selection ${isOpen ? 'is-open' : ''}`, children: [jsxRuntime.jsx("label", { for: viewSelectId, id: viewLabelId, className: "sx__view-selection-label", children: $app.translate('View') }), jsxRuntime.jsxs("button", { id: viewSelectId, type: "button", "aria-describedby": viewLabelId, "aria-label": $app.translate('Select View'), className: "sx__view-selection-selected-item sx__ripple", onClick: () => setIsOpen(!isOpen), onKeyDown: handleSelectedViewKeyDown, children: [selectedViewLabel, jsxRuntime.jsx("img", { className: "sx__view-selection-chevron", src: img, alt: "" })] }), isOpen && (jsxRuntime.jsx("ul", { "data-testid": "view-selection-items", className: "sx__view-selection-items", children: availableViews.map((view) => (jsxRuntime.jsx("li", { children: jsxRuntime.jsx("button", { type: "button", "aria-label": $app.translate('Select View') +
                              ' ' +
                              $app.translate(view.label), tabIndex: -1, onKeyDown: (keyboardEvent) => navigateUpOrDown(keyboardEvent, view.name), onClick: () => handleClickOnSelectionItem(view.name), className: 'sx__view-selection-item' +
                              (view.name === $app.calendarState.view.value
                                  ? ' is-selected'
                                  : ''), children: $app.translate(view.label) }) }, view.name))) }))] }));
  }
  ...
  function TimeAxis() {
      const $app = hooks.useContext(AppContext);
      const [gridSteps, setGridSteps] = hooks.useState(() => computeGridSteps($app.config.dayBoundaries.value, $app.config.isHybridDay, $app.config.weekOptions.value.gridStep));
      signals.useSignalEffect(() => {
  ...
  function useFetchEvents($app) {
      ...
      const fetchAndSetEvents = async () => {
          ...
          if (currentRange) {
              (_c = (_b = $app.config.plugins.eventRecurrence) === null || _b === void 0 ? void 0 : _b.onRangeUpdate) === null || _c === void 0 ? void 0 : _c.call(_b, currentRange);
          }
      };
      hooks.useEffect(() => {
          var _a;
  ...
  class CalendarConfigImpl {
      constructor(locale = DEFAULT_LOCALE, firstDayOfWeek = DEFAULT_FIRST_DAY_OF_WEEK, defaultView = InternalViewName.Week, views = [], dayBoundaries = DEFAULT_DAY_BOUNDARIES, weekOptions, calendars = {}, plugins = {}, isDark = false, isResponsive = true, callbacks = {}, _customComponentFns = {}, minDate = undefined, maxDate = undefined, monthGridOptions = {
      ...
      }, theme = undefined, translations = {}, showWeekNumbers = false, timezone = 'UTC', resources = [], resourceGridOptions = { nDays: 7 }, skipAnimations = false) {
          ...
          this.monthAgendaOptions = signals.signal(monthAgendaOptions);
          this.theme = theme;
